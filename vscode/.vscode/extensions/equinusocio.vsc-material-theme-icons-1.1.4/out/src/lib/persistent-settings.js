...
class PersistentSettings {
    ...
    getSettings() {
        ...
        if (!fs_1.existsSync(this.globalStoragePath)) {
            fs_1.mkdirSync(this.globalStoragePath);
        }
    ...
    getOldPersistentSettingsPath(isInsiders, isOSS, isDev) {
        ...
        return path_1.join(vscodeAppUserPath, constants_1.FILES.persistentSettings);
    }
    migrateOldPersistentSettings(isInsiders, isOSS, isDev) {
        ...
        if (fs_1.existsSync(oldPersistentSettingsFilePath)) {
            let oldState = require(oldPersistentSettingsFilePath);
            this.setState(oldState);
            fs_1.unlinkSync(oldPersistentSettingsFilePath);
        }
    }
    getState() {
